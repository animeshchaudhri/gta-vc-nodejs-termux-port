...
(function() {
    ...
    const cheatGamepadButtonIndex = 8; // Xbox View/Back button
    let cheatGamepadToggleWasPressed = false;
    ...
    function wrapCheatAwareHandler(h) {
        if (!h || !h.handlerFunc || h._wrapped) return;
    ...
    // Intercept game events after runtime input system is ready.
    function attachJSEventsHooks() {
        const jsEvents = globalThis.JSEvents;
        if (!jsEvents || typeof jsEvents.registerOrRemoveHandler !== 'function') {
            return false;
        }

        if (!jsEvents._cheatRegisterWrapped) {
            const originalRegister = jsEvents.registerOrRemoveHandler;
            jsEvents.registerOrRemoveHandler = function(h) {
                wrapCheatAwareHandler(h);
                return originalRegister.apply(this, arguments);
            };
            jsEvents._cheatRegisterWrapped = true;
        }

        if (jsEvents.eventHandlers) {
            jsEvents.eventHandlers.forEach(wrapCheatAwareHandler);
        }

        return true;
    }

    if (!attachJSEventsHooks()) {
        const waitForJSEvents = setInterval(() => {
            if (attachJSEventsHooks()) {
                clearInterval(waitForJSEvents);
            }
        }, 50);
    }
    ...
    function triggerCheatHaptics(gamepad) {
        try {
            // Modern gamepad haptics (supported on some browsers/controllers)
            ...
            // Legacy haptic actuators fallback
            if (gamepad && gamepad.hapticActuators && gamepad.hapticActuators[0] && typeof gamepad.hapticActuators[0].pulse === 'function') {
                gamepad.hapticActuators[0].pulse(0.6, 90).catch(() => {});
                return;
            }

            // Fallback for devices with Vibration API
    ...
    function getPressedCheatGamepad() {
        if (typeof navigator.getGamepads !== 'function') return null;
        const gamepads = navigator.getGamepads();
        for (let i = 0; i < gamepads.length; i++) {
            const gamepad = gamepads[i];
            if (!gamepad || !gamepad.buttons || !gamepad.buttons[cheatGamepadButtonIndex]) continue;
            if (gamepad.buttons[cheatGamepadButtonIndex].pressed) return gamepad;
        }
        return null;
    }

    function pollCheatGamepadToggle() {
        const pressedGamepad = getPressedCheatGamepad();
        const isPressed = !!pressedGamepad;
        if (isPressed && !cheatGamepadToggleWasPressed) {
            triggerCheatHaptics(pressedGamepad);
            toggleMenu();
        }
        cheatGamepadToggleWasPressed = isPressed;
        requestAnimationFrame(pollCheatGamepadToggle);
    }
    ...
    // Controller toggle (Xbox View/Back button)
    pollCheatGamepadToggle();
    ...
    window.typeCheat = async function(code) {
        ...
        const jsEvents = globalThis.JSEvents;
        if (!jsEvents || !jsEvents.eventHandlers) return;
