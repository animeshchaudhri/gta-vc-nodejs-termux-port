...
const gamepadButtonNameToIndex = {
    a: 0,
    b: 1,
    x: 2,
    y: 3,
...
function parseGamepadButtonIndex(value, fallback) {
    if (value == null || value === '') return fallback;

    const normalized = String(value).trim().toLowerCase();
    if (/^\d+$/.test(normalized)) {
        const parsed = parseInt(normalized, 10);
        return (parsed >= 0 && parsed <= 16) ? parsed : fallback;
    }

    return gamepadButtonNameToIndex[normalized] ?? fallback;
}

// Optional remap aliases for physical controllers (examples: ?gp_right_btn=rb or ?gp_right_btn=5)
const gameplayRightButtonIndex = parseGamepadButtonIndex(params.get('gp_right_btn'), 7);
const gameplayLeftButtonIndex = parseGamepadButtonIndex(params.get('gp_left_btn'), 6);
const gameplayDebugButtonsEnabled = params.get('gp_debug') !== '0';
...
function applyGamepadTriggerAliases() {
    if (gameplayRightButtonIndex === 7 && gameplayLeftButtonIndex === 6) return;
    ...
    navigator.getGamepads = function() {
        ...
        const remapped = Array.from(gamepads);
        for (let i = 0; i < remapped.length; i++) {
            const gp = remapped[i];
            if (!gp || !gp.buttons || !gp.buttons.length) continue;

            const clonedButtons = Array.from(gp.buttons, (btn) => ({
                pressed: !!btn?.pressed,
                value: btn?.value || 0,
                touched: !!btn?.touched,
            }));

            const aliasButton = (targetIndex, sourceIndex) => {
                if (sourceIndex === targetIndex) return;
                const source = clonedButtons[sourceIndex];
                const target = clonedButtons[targetIndex];
                if (!source || !target) return;
                if (source.pressed || source.value > 0 || source.touched) {
                    clonedButtons[targetIndex] = {
                        pressed: target.pressed || source.pressed,
                        value: Math.max(target.value, source.value),
                        touched: target.touched || source.touched,
                    };
                }
            };

            aliasButton(7, gameplayRightButtonIndex);
            aliasButton(6, gameplayLeftButtonIndex);
...
function triggerGameplayHaptics(gamepad, weak = 0.4, strong = 0.6, duration = 70) {
    try {
        ...
        if (gamepad?.hapticActuators?.[0]?.pulse) {
            gamepad.hapticActuators[0].pulse(Math.max(weak, strong), duration).catch(() => {});
            return;
        }
        if (navigator.vibrate) {
            navigator.vibrate(Math.min(duration, 80));
        }
    } catch (e) {}
}

function startGameplayHapticsLoop() {
    if (typeof navigator.getGamepads !== 'function') return;

    let wasFirePressed = false;
    let lastPulseAt = 0;
    ...
    const tick = () => {
        ...
        for (let i = 0; i < gamepads.length; i++) {
            const gp = gamepads[i];
            if (!gp || !gp.buttons || gp.buttons.length < 8) continue;

            const rightPressed = !!gp.buttons[7]?.pressed;
            const leftPressed = !!gp.buttons[6]?.pressed;
        ...
        if (firePressed && !wasFirePressed && now - lastPulseAt > 120) {
            triggerGameplayHaptics(activeGamepad, 0.35, 0.65, 70);
            lastPulseAt = now;
        }
...
function startGamepadButtonDebugOverlay() {
    if (!gameplayDebugButtonsEnabled || typeof navigator.getGamepads !== 'function') return;
    ...
    overlay.textContent = 'GP Debug: waiting for controller';
        ...
        for (let i = 0; i < gamepads.length; i++) {
            ...
            for (let buttonIndex = 0; buttonIndex < gp.buttons.length; buttonIndex++) {
                if (gp.buttons[buttonIndex]?.pressed) {
                    const name = gamepadIndexToName[buttonIndex] || 'Button';
                    pressed.push(`${buttonIndex} (${name})`);
                }
            }

            if (pressed.length > 0) {
                lines.push(`Pad ${i}: ${pressed.join(', ')}`);
            }
        }

        if (lines.length > 0) {
            overlay.style.opacity = '1';
            overlay.textContent = `GP Debug\n${lines.join('\n')}\nMap: ?gp_right_btn=<index|name>&gp_left_btn=<index|name>\nNames: rt/rb, lt/lb`;
        } else {
            overlay.style.opacity = '0.55';
            overlay.textContent = 'GP Debug: press a controller button';
        }
...
let fpsOverlayStarted = false;

function startFpsOverlay() {
    if (fpsOverlayStarted) return;
    fpsOverlayStarted = true;
    ...
    overlay.style.background = 'rgba(0, 0, 0, 0.65)';
    overlay.style.color = '#9dffb0';
    overlay.style.padding = '6px 10px';
    overlay.style.font = '12px/1.25 monospace';
    overlay.style.border = '1px solid rgba(157, 255, 176, 0.5)';
    overlay.style.borderRadius = '6px';
    overlay.style.pointerEvents = 'none';
    ...
    const tick = (now) => {
        ...
        // Ignore very large gaps from tab switch/suspend so average stays meaningful.
...
async function loadGame(data) {
    ...
    emulator.AddDisplayButtonEventListeners(0, [{
        buttonIndex: gameplayRightButtonIndex,
        lockTargetWhilePressed: false,
    ...
    emulator.AddDisplayButtonEventListeners(0, [{
        buttonIndex: gameplayRightButtonIndex,
        buttonIndexes: [1, gameplayRightButtonIndex],
        lockTargetWhilePressed: false,
        tapTarget: document.querySelector('.touch-control.fireRight'),
    }]);
    emulator.AddDisplayButtonEventListeners(0, [{
        buttonIndex: gameplayLeftButtonIndex,
        buttonIndexes: [1, gameplayLeftButtonIndex],
        lockTargetWhilePressed: false,
        tapTarget: document.querySelector('.touch-control.fireLeft'),
    }]);

    startGameplayHapticsLoop();
    startGamepadButtonDebugOverlay();
}
